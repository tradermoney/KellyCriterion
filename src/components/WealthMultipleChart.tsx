...
interface WealthMultipleChartProps {
  summaries: StrategySummary[];
  height?: number;
}

export const WealthMultipleChart: React.FC<WealthMultipleChartProps> = ({
  summaries,
  height = 300
}) => {
  const { t } = useLanguage();
  ...
  const chartData = React.useMemo(() => {
    ...
    // 计算每个策略的平均财富倍数路径
    const avgMultiplePaths = summaries.map(summary => {
      ...
      // 计算每个时间点的平均财富倍数（相对于初始财富）
      const avgMultiplePath = [];
      for (let i = 0; i < maxLength; i++) {
        ...
        summary.paths.forEach(path => {
          if (path.wealthHistory && i < path.wealthHistory.length) {
            // 财富倍数 = 当前财富 / 初始财富
            const wealthMultiple = path.wealthHistory[i] / path.wealthHistory[0];
            sum += wealthMultiple;
            count++;
          }
        });
        ...
        if (count > 0) {
          avgMultiplePath.push(sum / count);
        }
      }

      return avgMultiplePath;
    });

    // 找到最长的路径
    const maxLength = Math.max(...avgMultiplePaths.map(path => path.length));
    if (maxLength === 0) return [];
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      summaries.forEach((summary, index) => {
        if (i < avgMultiplePaths[index].length) {
          const strategyName = generateStrategyDisplayName(summary.strategy, index);
          point[strategyName] = avgMultiplePaths[index][i];
        }
      });
